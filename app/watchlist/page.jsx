...
export default function Page() {
  ...
  async function fetchMovie() {
    if (session && session.user) {
      const req = await fetch(
        `https://api.themoviedb.org/3/account/${session.user.id}/watchlist/movies?api_key=${process.env.NEXT_PUBLIC_API_KEY}&session_id=${tmdb_session}`
      );
  ...
  async function fetchTv() {
    if (session && session.user) {
      const req = await fetch(
        `https://api.themoviedb.org/3/account/${session.user.id}/watchlist/tv?api_key=${process.env.NEXT_PUBLIC_API_KEY}&session_id=${tmdb_session}`
      );
      const res = await req.json();
      console.log(res);
      setTv(res.results);
  ...
  useEffect(() => {
    fetchMovie();
    fetchTv();
    console.log(tmdb_session, session);
          ...
          {tv && tv.length < 1 && movie && movie.length < 1 && (
            <>
              <h1 className="text-white text-5xl text-center mt-52">
                You have not any movie or series to watchlist
              </h1>
            </>
          )}
        </div>
